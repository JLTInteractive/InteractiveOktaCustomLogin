...
juis.tabList = (function() {
  ...
  function _setup() {
    ...
    var tabListAnchors = document.querySelectorAll(".g-tab__tab-list a");
    ...
    var tabGroups = document.querySelectorAll(".g-tab__tab-group");

    //initialise tabs
    for (var i=0; i < tabGroups.length; i++) {
      //hide tab panels (for all tab groups)
      var tabPanels = tabGroups[i].querySelectorAll(".g-tab__tab-panel");
      ...
      for (var j=0; j < tabPanels.length; j++) {
        ...
        var associatedTab = document.querySelector(".g-tab__tab a[href='#" + tabPanels[j].id + "']");

        //if tab panel ID is not in currentTabs
        if (currentTabs.indexOf("#" + tabPanels[j].id) === -1) {
          //hide it
          juis.setVisibility(tabPanels[j]);
      ...
      juis.setVisibility(tabPanels[0], true);
      tabPanels[0].setAttribute("tabindex", "0");
      tabPanels[0].setAttribute("role", "tabpanel");

      //select first tab
      var firstTab = tabGroups[i].querySelectorAll(".g-tab__tab")[0];
